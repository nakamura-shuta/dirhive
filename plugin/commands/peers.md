...
allowed-tools:
  - mcp__dirhive__sync.list-peers
---

Call `mcp__dirhive__sync.list-peers` and present the result.
...
- If any peer has `last_seen_at == null`, hint that the other side may not have completed their `accept` / `allow-peer` step, or that no edits have flowed yet. Suggest verifying with `/dirhive:status` on both sides.
- An empty list means no peer is currently allowed. Use `/dirhive:invite` or `/dirhive:accept` to bootstrap.
