...
allowed-tools:
  - mcp__p2p-dir-sync__sync.revoke
---
...
If the user did not provide a peer_id, stop and ask them for it explicitly (e.g. via `/p2p-dir-sync:peers` first).

Call `mcp__p2p-dir-sync__sync.revoke` with `{peer_id: $1}`.
...
- For a stronger revoke (= rotate the group identity), delete `~/.local/share/p2p-dir-sync/folder-secret.bin` on every honest peer and re-bootstrap with new invites. There is no in-band "rotate" yet.
- Verify with `/p2p-dir-sync:peers` that the entry is gone.
