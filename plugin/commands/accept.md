...
allowed-tools:
  - mcp__dirhive__sync.accept-invite
---

Parse the arguments:

- `$1` = the ticket string (starts with `dirhive1-`)
...
Call `mcp__dirhive__sync.accept-invite` with `{ticket: $1, label: $2}`.
...
2. Tell the inviter to run `/dirhive:allow-peer <my_peer_id>` on their side, supplying *your* `my_peer_id` value.
3. Once their `allow-peer` is done, both sides are in the mesh and edits should start propagating. Verify with `/dirhive:peers`.

If `accept-invite` returns an error mentioning a different `folder_secret`, the daemon is already initialised with another group. Either keep using the existing group, or delete `~/.local/share/dirhive/folder-secret.bin` and restart to reset (this wipes the current group identity).
