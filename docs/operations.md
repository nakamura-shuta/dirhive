# operations.md — p2p-dir-sync 運用手順

`p2p-sync` daemon を実機で長期運用するときの手順集。 **新規 user の install から、 launchd 常駐、 invite/accept、 トラブルシュート、 復旧、 uninstall まで** をこの 1 doc にまとめる。
...
```sh
cd /path/to/p2p-dir-sync
./plugin/scripts/install.sh
```

何が起きるか:

1. `cargo build --release --bin p2p-sync --bin p2p-sync-mcp`
2. `~/.local/bin/p2p-sync` と `~/.local/bin/p2p-sync-mcp` を 0755 で配置
...
env override:
- `P2P_SYNC_LOG=info,p2p_dir_sync=debug` — tracing filter
- `P2P_SYNC_STATE_DIR=...` — `~/.local/share/p2p-dir-sync` の上書き (= test 用)
- `P2P_SYNC_CONFIG_DIR=...` — `~/.config/p2p-dir-sync` の上書き
- `P2P_SYNC_LOG_DIR=...` — `~/Library/Logs` の上書き
...
| 出力先 | 中身 |
|---|---|
| `~/Library/Logs/p2p-dir-sync.log` | daemon 本体の file appender (= `sync.recent-log` が tail する file、 secrets 自動 redact) |
| `~/Library/Logs/p2p-dir-sync.stdout.log` | launchd の stdout redirect (= 普段空) |
| `~/Library/Logs/p2p-dir-sync.stderr.log` | launchd の stderr redirect (= cargo build 等の出力もここ) |

AI agent からは `/p2p-dir-sync:setup-doctor` の step 4 や `sync.recent-log` で覗ける (= secrets redact 済)。
...
```sh
SOCK=~/.local/share/p2p-dir-sync/daemon.sock
...
**foreground** で動かしている場合: Ctrl+C → 再度 `p2p-sync --watch ...`
...
launchctl kickstart -k gui/$UID/com.user.p2p-dir-sync   # Carol 再起動
...
rm ~/.local/share/p2p-dir-sync/folder-secret.bin
launchctl kickstart -k gui/$UID/com.user.p2p-dir-sync
...
```sh
/p2p-dir-sync:setup-doctor   # 4-step probe
# または
...
```sh
launchctl print gui/$UID/com.user.p2p-dir-sync 2>/dev/null | head -3
...
```sh
rm ~/.local/share/p2p-dir-sync/daemon.sock
rm ~/.local/share/p2p-dir-sync/daemon.lock  # flock を保持する file
```
...
```sh
rm ~/.config/p2p-dir-sync/endpoint.key
launchctl kickstart -k gui/$UID/com.user.p2p-dir-sync
# → 新規 EndpointId で起動
...
backup file の中身が無事だったら手動で merge して 1 つに戻し、 backup file は削除する。 daemon は backup file を broadcast 対象に含めない (= watcher の `*.conflict-local-*` フィルタは入っていないので、 もし sync に流したくなければ手動で削除推奨)。

### 5.6 「 daemon が高頻度で restart している (= crash loop) 」

`launchctl print gui/$UID/com.user.p2p-dir-sync` で `last_exit_code` / `last_exit_status` を見る。 永続的に同じ exit code なら原因あり:

```sh
tail -100 ~/Library/Logs/p2p-dir-sync.stderr.log
```
...
`ThrottleInterval=10` で 10s sleep してから再起動するので、 デバッグ中はその間に foreground で `p2p-sync --watch ...` を試して error 内容を見る方が早い。
...
```sh
rm -rf ~/.local/share/p2p-dir-sync   # blobs + allowlist + pending_log + folder_secret
rm -rf ~/.config/p2p-dir-sync        # endpoint.key
rm -f  ~/Library/Logs/p2p-dir-sync.{,stdout.,stderr.}log
```
...
| path | 役割 |
|---|---|
| `~/.local/bin/p2p-sync` | daemon binary |
| `~/.local/bin/p2p-sync-mcp` | MCP server binary (= AI agent 経路) |
| `~/.local/share/p2p-dir-sync/daemon.sock` | Unix socket (0o600) |
| `~/.local/share/p2p-dir-sync/daemon.lock` | flock 用 file (0o600、 多重起動防止) |
| `~/.local/share/p2p-dir-sync/folder-secret.bin` | folder secret (= group identity、 0o600) |
| `~/.local/share/p2p-dir-sync/allowlist.json` | peer allowlist (0o600) |
| `~/.local/share/p2p-dir-sync/bootstrap-peers.json` | gossip subscribe 用 bootstrap peer addr (= accept-invite で永続化) |
| `~/.local/share/p2p-dir-sync/blobs/` | iroh-blobs FsStore (= 同期した file の中身) |
| `~/.local/share/p2p-dir-sync/pending/<repo_hash>/*.json` | 受信 change log |
| `~/.config/p2p-dir-sync/endpoint.key` | Ed25519 secret (= peer identity、 0o600) |
| `~/Library/Logs/p2p-dir-sync.log` | daemon file appender (= secrets redact 済、 `sync.recent-log` の source) |
| `~/Library/Logs/p2p-dir-sync.{stdout,stderr}.log` | launchd の redirect 先 |
| `~/Library/LaunchAgents/com.user.p2p-dir-sync.plist` | launchd user agent 定義 |
