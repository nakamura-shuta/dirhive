...
```
dirhive/                        # 独立 repo (jj/git colocated)
├── Cargo.toml
├── README.md
├── docs/
│   ├── requirements.md
...
│   │   └── client.rs                # rpc()
│   └── bin/
│       ├── dirhive.rs              # daemon binary (= L2)
│       └── dirhive-mcp.rs          # MCP server binary (= L3)
...
socket path: `~/.local/share/dirhive/daemon.sock` (mode 0o600)、newline-delimited JSON。
...
| `sync.recent-log` | `{lines?: u32 ≤ 100}` | `{lines: [String...]}` | `~/Library/Logs/dirhive.log` の末尾、ticket / folder_secret は `<redacted>` 置換 |

`sync.ping` は daemon RPC に存在せず、MCP server 内で完結する (§3.3)。

### 3.3 MCP tool (`dirhive-mcp` binary)
...
[Alice]
6. /dirhive:allow-peer <Bob_id> [--label Bob]
...
wire format = `dirhive1-<base32(serde_json::to_string(&InviteTicket))>`。

- prefix `dirhive1-`: schema version 1。将来 v2 で field 追加なら `p2psync2-` に bump
...
fn derive_topic_id(folder_secret: &[u8; 16]) -> TopicId {
    ...
    hasher.update(b"dirhive/v1/topic\0");
...
~/Library/Logs/
└── dirhive.log           # daemon log (macOS 慣習)
```

## 5. ライフサイクル

### 5.1 daemon 起動 (`dirhive --watch <dir>`)
...
`sandbox/scripts/launchd/com.user.dirhive.plist`:
...
<key>StandardOutPath</key> <string>~/Library/Logs/dirhive.stdout.log</string>
<key>StandardErrorPath</key> <string>~/Library/Logs/dirhive.stderr.log</string>
```

**StandardOutPath / StandardErrorPath は daemon 本体の log file (= `~/Library/Logs/dirhive.log`) と別 path にする** (= `*.stdout.log` / `*.stderr.log` で suffix を分ける)。理由: daemon 本体が `init_tracing` で `dirhive.log` に file appender を張っているので、launchd の stderr redirect 先が同じ file になると同じ行が 2 回書かれる。 `sync.recent-log` が tail するのは daemon の file appender 出力 (= `dirhive.log`) のみ。 launchd 側の 2 file は launchd プロセス管理の crash diagnostics 用と位置付ける。
...
| peer 認可 | gossip mesh join 制限 | topic_id = `BLAKE3("dirhive/v1/topic\0" ‖ folder_secret)`、第三者は導出不能 |
...
| 入力 validation | RPC 引数 | `accept-invite` ticket は `dirhive1-` prefix + base32 decode + InviteTicket deserialize、`allow-peer` / `revoke` の peer_id は `EndpointId::from_str` 失敗 → reject |
...
- **ticket は機密扱い**: `dirhive1-` envelope に folder_secret を含む。漏洩 = mesh 全体への join 権限漏洩。配布経路 (Slack / iMessage 等) は user の責任で confidential を保つ
...
- **gossip allowlist は DoS layer であって adversary layer ではない**: `SyncUpdate.from.id` は payload 内の自己申告で署名検証していない。folder_secret を持つ peer は他 peer の `from.id` を詐称できる (特に Tombstone は blob fetch なしで削除まで通る)。MVP では「folder_secret = mesh 入場券 = 信頼境界」とみなし、allowlist は既知 peer 管理 + 未許可 peer noise の DoS 抑止に位置付ける。真の認証境界 (= folder_secret 漏洩後の悪意化 peer 耐性) は Phase 3+ で SyncUpdate を `from.id` の secret_key で署名 + 受信時公開鍵検証する形で対応予定
...
✓ Step 2 sync.health-check    : daemon connected
    socket             : ~/.local/share/dirhive/daemon.sock (0o600)
    key                : ~/.config/dirhive/endpoint.key (32 bytes)
...
- Phase 4 完了: `dirhive-mcp` initialize round-trip
...
| Q5 plugin 名 | `dirhive`、namespaced slash command は `/dirhive:status` 等 |
| Q6 初期対象 macOS | launchd staging のみ、systemd / Windows Service は dir 構造で余地を残す |
| Q7 peer secrecy | folder_secret 由来 topic + AllowlistBlobs wrap。`dirhive1-<base32>` envelope |
